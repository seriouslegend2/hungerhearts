...
orderSchema.pre("save", async function (next) {
    try {
        const donorExists = await Donor.exists({
            username: this.donorUsername,
        });
        if (!donorExists)
            throw new Error(
                `Donor with username ${this.donorUsername} does not exist`
            );

        const userExists = await User.exists({ username: this.userUsername });
        if (!userExists)
            throw new Error(
                `User with username ${this.userUsername} does not exist`
            );

        const deliveryBoyExists = await DeliveryBoy.exists({
            _id: this.deliveryBoy,
        });
        if (!deliveryBoyExists)
            throw new Error(
                `DeliveryBoy with ID ${this.deliveryBoy} does not exist`
            );
