...
export const getModerators = async (req, res) => {
    try {
        ...
        if (!mod) {
            return res
                .status(400)
                .json({ message: "Invalid moderator credentials" });
        }
        ...
        res.status(500).json({
            message: "Server error in retrieving moderators",
        });
    }
};
...
export const getDonors = async (req, res) => {
    try {
        ...
        const response = await axios.get(
            "http://localhost:9500/donor/getDonors",
            {
                headers: {
...
export const changeRole = async (req, res) => {
    try {
        ...
        if (!updatedMod) {
            return res
                .status(404)
                .json({ success: false, message: "Mod not found" });
        }
